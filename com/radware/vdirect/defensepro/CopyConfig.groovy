...
class CopyConfig {
    ...
    String SyncAction(
            ...
            @Param(name = 'syncBlackWhiteList', type = 'bool',
                    prompt = 'Sync Black\\White lists (includes NetworkClasses)', uiEditable = "true",
            ...
            @Param(name = 'srvHttpBaseline', type = 'bool', prompt = 'HTTP Baselines (DP Ver. 6 & 7)',
                    defaultValue = 'false') boolean srvHttpBaseline,
        ...
        try {
            ...
            if (syncSslCertificates){
                ...
                deviceConnectionList.each {dp ->
                    ...
                    if (srcCertBeans.size() > 0) {
                        ...
                        log.info String.format("Certs on devices %s, %s are identical", srcDP.getManagementIp(), dstDP.getManagementIp())
                    }
                }
            }
            ...
            if (syncBlackWhiteList) {
                StringBuffer sb = new StringBuffer()
                RsBWMNetworkEntry srcBwmNetworkEntry = new RsBWMNetworkEntry()
                List<RsBWMNetworkEntry> srcBeans = (List<RsBWMNetworkEntry>) masterDevice.readAll(srcBwmNetworkEntry)
                if (!srcBeans.isEmpty()) {
                        ...
                        if (dstBeans.isEmpty()) {
                            ...
                            for (RsBWMNetworkEntry srcBean : srcBeans) {
                                ...
                                if (!foundMatchingBean) {
                                    ...
                                    if (dp.isEmpty(entry2)) {
                                        log.debug String.format("dest device has no index %s for obj %s", srcBean.getSubIndex(), srcBean.getName())
                                        dp.create(srcBean)
                                    } else {
            ...
            * */
                ...
                List<RsNewWhiteListEntry> srcWLBeans = (List<RsNewWhiteListEntry>) masterDevice.readAll(srcWhiteListEntry)
                ...
                List<RsNewBlackListEntry> srcBLBeans = (List<RsNewBlackListEntry>) masterDevice.readAll(srcBlackListEntry)
                ...
                List<RsBWMAppPortGroupEntry> srcAppPortGroupBeans = (List<RsBWMAppPortGroupEntry>) masterDevice.readAll(srcAppPortGroupEntry)
                    ...
                    if (!srcAppPortGroupBeans.isEmpty()) {
                        ...
                        if (dstAppPortGroupBeans.isEmpty()) {
                            ...
                            for (RsBWMAppPortGroupEntry srcBean : srcAppPortGroupBeans) {
                                ...
                                if (!foundMatchingBean && srcBean.getType().toString() != "STATIC") {
